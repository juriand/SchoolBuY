...
import com.schoolbuy.utils.FaceTextUtils;
import com.schoolbuy.utils.ImageLoadOptions;
import com.schoolbuy.utils.TimeUtil;
...
public class MessageChatAdapter extends BaseListAdapter<BmobMsg> {
	...
	private View createViewByType(BmobMsg message, int position) {
	   ...
	   if(type==BmobConfig.TYPE_VOICE){//语音类型
	...
	public View bindView(final int position, View convertView, ViewGroup parent) {
		...
		if(avatar!=null && !avatar.equals("")){//加载头像-为了不每次都加载头像
			...
			iv_avatar.setImageResource(R.drawable.icon_noset);
		}
		
		tv_time.setText(TimeUtil.getChatTime(Long.parseLong(item.getMsgTime())));
		
		if(getItemViewType(position)==TYPE_SEND_TXT||getItemViewType(position)==TYPE_SEND_VOICE){//只有自己发送的消息才有重发机制
			//状态描述
